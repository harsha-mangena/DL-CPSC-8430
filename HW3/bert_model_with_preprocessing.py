...
token_lens = [len(context.strip().split(' ')) for context in train_contexts]
...
ans_lens = [len(question.strip().split(' ')) for question in train_questions]

plt.figure()
plt.hist(ans_lens, bins=20)
plt.ylabel('Count')
plt.xlabel('Length')
...
print("WER( with preprocessing): ", wer_list)
