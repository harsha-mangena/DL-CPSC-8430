...
plt.imshow(np.transpose(utils.make_grid(real_batch[0].to(device)[:40],nrow=5, padding=2, normalize=True).cpu(),(1,2,0)))
...
for epoch in range(NUM_EPOCHS):
    ...
    print(f"Epoch [{epoch}/{NUM_EPOCHS}] Batch {batch_idx}/{len(dataloader)}                   Loss Discriminator: {loss_critic:.3f}, loss Generator: {loss_gen:.3f}, FID Score: {fretchet_dist:.3f} ", end="\r", flush=True) 
...
real_images = utils.make_grid(real_batch[0].to(device)[:64], padding=1, normalize=True).cpu()
plt.imshow(np.transpose(real_images, (1, 2, 0)))
...
plt.savefig('Results/WGANGP_BEST_FAKE_32.png', format='png', dpi=100, bbox_inches='tight')
